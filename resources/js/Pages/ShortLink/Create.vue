    ...
    <AppLayout>
        ...
        <div class="max-w-2xl mx-auto">
            ...
            <div class="flex items-center gap-3 mb-8">
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-900 dark:text-white">Create Short Link</h1>
                    <p class="text-sm text-gray-400 dark:text-gray-500 mt-0.5">Shorten a URL and track its performance</p>
                </div>
            </div>
                ...
                <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <label class="flex items-center gap-3 cursor-pointer select-none group">
                            <div class="relative shrink-0">
                                ...
                                <div :class="useCustomAlias ? 'bg-primary-600' : 'bg-gray-200 dark:bg-gray-700'" class="w-9 h-5 rounded-full transition-colors"></div>
                                <div :class="useCustomAlias ? 'translate-x-4' : 'translate-x-0'" class="absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform"></div>
                            </div>
                            <span class="text-sm text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-200 transition-colors">Use a custom alias</span>
                        </label>
                        ...
                        <Transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 -translate-y-1" enter-to-class="opacity-100 translate-y-0">
                            <div v-if="useCustomAlias" class="space-y-1.5">
                    ...
                    <div class="p-6 space-y-4">
                        <label class="flex items-center gap-3 cursor-pointer select-none group">
                            ...
                            <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer">QR Code</label>
                        </label>

                        <Transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 -translate-y-1" enter-to-class="opacity-100 translate-y-0">
                            <div v-if="showQr" class="space-y-3">
                                <button type="button" @click="previewQr" :disabled="!form.original_url || loadingQr"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"/></svg>
                                    {{ loadingQr ? 'Loading…' : 'Preview QR' }}
                                </button>
                                <div v-if="qrDataUrl" class="flex justify-center">
                                    <img :src="qrDataUrl" alt="QR preview" class="w-36 h-36 rounded-xl border border-gray-200 dark:border-gray-700" />
                                </div>
                                <div v-else class="flex flex-col items-center justify-center h-20 rounded-xl bg-gray-50 dark:bg-gray-800 gap-1.5">
                                    <svg class="w-5 h-5 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"/></svg>
                                    <span class="text-xs text-gray-400">Enter a URL and click Preview QR</span>
                                </div>
                                <p class="text-xs text-gray-400 text-center">High-res QR download available after saving</p>
                            </div>
                        </Transition>
                    </div>
